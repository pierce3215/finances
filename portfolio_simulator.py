...
def get_portfolio_data():
    ...
    return pd.DataFrame(records), total_value
...
def update_portfolio(_):
    ...
    if df.empty or 'Ticker' not in df.columns or 'Value' not in df.columns:
        empty_fig = px.pie(values=[1], names=["No Data"], title="Portfolio Data Unavailable")
        empty_fig.update_layout(template="plotly_dark", font_color="white")
        return empty_fig, html.Div("⚠️ Failed to fetch portfolio data. Please try again later.", style={"color": "red", "textAlign": "center"})
